...
#ifdef CONFIG_ARCH_LYNX_GP11D
#define SET_LEFT_X 0
#define SET_RIGHT_X 600
#define SET_TOP_Y 1600
#define SET_BOTTOM_Y 1919
...
#ifdef CONFIG_QPNP_SCHAPTIC
	static bool set_auto_res_mode = false;
	static u8 lra_auto_res_reg = 0x00;
	static u8 original_param[9] = {0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};
...
static int qpnp_hap_set(struct qpnp_hap *hap, int on)
{
	...
	int touch_area_y = -1;
	u8 set_vmax_param = 0;
	u8 temp_vmax_param = 0;
	bool param_change_area_detect = false;
#endif
	int i;
		...
		if (on) {
			...
			if( (bootmode == SH_BOOT_D) || (bootmode == SH_BOOT_F_F) ) {
				...
				if (smemzero == false) {
					...
					rc = qpnp_hap_read_reg(hap, &original_param[1], QPNP_HAP_VMAX_REG(hap->base));
					if (rc < 0)
						return rc;
					rc = qpnp_hap_write_reg(hap, &param[1], QPNP_HAP_VMAX_REG(hap->base));
					if (rc)
						return rc;
					rc = qpnp_hap_read_reg(hap, &original_param[2], QPNP_HAP_RATE_CFG1_REG(hap->base));
					if (rc < 0)
						return rc;
					rc = qpnp_hap_write_reg(hap, &param[2], QPNP_HAP_RATE_CFG1_REG(hap->base));
					if (rc)
						return rc;
					rc = qpnp_hap_read_reg(hap, &original_param[3], QPNP_HAP_RATE_CFG2_REG(hap->base));
					if (rc < 0)
						return rc;
					...
					if (rc)
						return rc;
					if (param[0] == 0) {
						rc = qpnp_hap_read_reg(hap, &original_param[0], QPNP_HAP_LRA_AUTO_RES_REG(hap->base));
						if (rc < 0)
							return rc;
					...
					if (set_auto_res_mode) {
						...
						mb();
						mdelay(20);
						rc = qpnp_hap_write_reg(hap, &lra_auto_res_reg, QPNP_HAP_LRA_AUTO_RES_REG(hap->base));
					} else {
						rc = qpnp_hap_mod_enable(hap, on);
						if (rc < 0)
							return rc;
...
#ifdef CONFIG_ARCH_LYNX_GP11D
				msm_tps_get_single_touchdown_info(&touch_area_x, &touch_area_y);
				if( ( (touch_area_x >= SET_LEFT_X) && (touch_area_x <= SET_RIGHT_X) ) &&  ( (touch_area_y >= SET_TOP_Y) && (touch_area_y <= SET_BOTTOM_Y) ) ) {
					param_change_area_detect = true;
					rc = qpnp_hap_read_reg(hap, &temp_vmax_param, QPNP_HAP_VMAX_REG(hap->base));
					if (rc < 0) {
						return rc;
					}
					set_vmax_param = 0x2C;
					rc = qpnp_hap_write_reg(hap, &set_vmax_param, QPNP_HAP_VMAX_REG(hap->base));
					if (rc) {
						return rc;
					}
				}
...
#ifdef CONFIG_ARCH_LYNX_GP11D
				if(param_change_area_detect) {
					param_change_area_detect = false;
					touch_area_x = -1;
					touch_area_y = -1;
					rc = qpnp_hap_write_reg(hap, &temp_vmax_param, QPNP_HAP_VMAX_REG(hap->base));
						if (rc) {
							return rc;
						}
				}
			...
			if(set_auto_res_mode) {
				...
				reg = 0x00;
				rc = qpnp_hap_write_reg(hap, &reg, QPNP_HAP_LRA_AUTO_RES_REG(hap->base));
			} else {
				rc = qpnp_hap_play(hap, on);
				if (rc < 0)
					return rc;
			...
			if( (bootmode == SH_BOOT_D) || (bootmode == SH_BOOT_F_F) ) {
				...
				if (param[0] | param[1] | param[2] | param[3])
					smemzero = false;
				if (smemzero == false) {
					rc = qpnp_hap_write_reg(hap, &original_param[0], QPNP_HAP_LRA_AUTO_RES_REG(hap->base));
					if (rc)
						return rc;
					rc = qpnp_hap_write_reg(hap, &original_param[1], QPNP_HAP_VMAX_REG(hap->base));
					if (rc)
						return rc;
					rc = qpnp_hap_write_reg(hap, &original_param[2], QPNP_HAP_RATE_CFG1_REG(hap->base));
					if (rc)
						return rc;
					rc = qpnp_hap_write_reg(hap, &original_param[3], QPNP_HAP_RATE_CFG2_REG(hap->base));
					if (rc)
						return rc;
				}
			}
...
#ifdef CONFIG_QPNP_SCHAPTIC
	if(!of_property_read_u32(spmi->dev.of_node, "qcom,en-auto-res-mode", &temp)) {
		set_auto_res_mode = true;
		lra_auto_res_reg = temp;
	}
