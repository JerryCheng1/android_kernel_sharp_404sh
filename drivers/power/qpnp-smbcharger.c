...
#ifdef CONFIG_BATTERY_SH
	unsigned int			*thermal_mitigation_hvdcp_vol_up;
	unsigned int			*thermal_mitigation_hvdcp_vol_down;
#endif /* CONFIG_BATTERY_SH */
...
static int sh_get_prop_batt_status(struct smbchg_chip *chip)
{
	...
	int charger_event = SHBATTLOG_EVENT_NONE;
	static int pre_charge_status_event = SHBATTLOG_EVENT_NONE;
	static int pre_charge_error_event = SHBATTLOG_EVENT_NONE;
	static int pre_charger_event = SHBATTLOG_EVENT_NONE;
	static bool pre_usb_present = false;
...
out:
	if( !pre_usb_present && is_usb_present(chip)) {
		if(chip->usb_psy) {
		...
		case POWER_SUPPLY_TYPE_USB_CDP:
			charger_event = SHBATTLOG_EVENT_CHG_INSERT_USB;
			break;
		case POWER_SUPPLY_TYPE_USB_DCP:
		case POWER_SUPPLY_TYPE_USB_HVDCP:
			charger_event = SHBATTLOG_EVENT_CHG_INSERT_CHGR;
			break;
		default:
			charger_event = SHBATTLOG_EVENT_NONE;
		}
		pre_usb_present = true;
		pre_charger_type = prop.intval;
		
	} else if(pre_usb_present && !is_usb_present(chip)) {
		if(chip->usb_psy) {
			chip->usb_psy->get_property(chip->usb_psy, POWER_SUPPLY_PROP_TYPE, &prop);
		}
		switch(pre_charger_type) {
		case POWER_SUPPLY_TYPE_USB:
		case POWER_SUPPLY_TYPE_USB_CDP:
			charger_event = SHBATTLOG_EVENT_CHG_REMOVE_USB;
			break;
		case POWER_SUPPLY_TYPE_USB_DCP:
		case POWER_SUPPLY_TYPE_USB_HVDCP:
			charger_event = SHBATTLOG_EVENT_CHG_REMOVE_CHGR;
			break;
		default:
			charger_event = SHBATTLOG_EVENT_NONE;
		}
		pre_usb_present = false;
		pre_charger_type = POWER_SUPPLY_TYPE_UNKNOWN;
		
	}

	pr_smb(PR_STATUS, "pre_charger_event = %d, charger_event = %d\n", pre_charger_event, charger_event);
	if(charger_event != SHBATTLOG_EVENT_NONE && charger_event != pre_charger_event) {
		shbatt_api_kernel_battery_log_info_event(charger_event, 0);
		pre_charger_event = charger_event;
	}
	...
	pr_smb(PR_STATUS, "pre_charge_status_event = %d, charge_status_event = %d\n", pre_charge_status_event, charge_status_event);
	if(charge_status_event != SHBATTLOG_EVENT_NONE && charge_status_event != pre_charge_status_event) {
		shbatt_api_kernel_battery_log_info_event(charge_status_event, 0);
		pre_charge_status_event = charge_status_event;
	}
	
	pr_smb(PR_STATUS, "pre_charge_error_event = %d, charge_error_event = %d\n", pre_charge_error_event, charge_error_event);
	if(charge_error_event != SHBATTLOG_EVENT_NONE && charge_error_event != pre_charge_error_event) {
		shbatt_api_kernel_battery_log_info_event(charge_error_event, 0);
		pre_charge_error_event = charge_error_event;
	}

	pr_smb(PR_STATUS, "jeita_pre_status = %d,jeita_cur_status  = %d\n", jeita_pre_status , jeita_cur_status );
	if (jeita_cur_status  != jeita_pre_status) {
		switch (jeita_cur_status){
		case POWER_SUPPLY_HEALTH_OVERHEAT:
			shbatt_api_kernel_battery_log_info_event(SHBATTLOG_EVENT_CHG_HOT_STOP_ST, 0);
			break;
		case POWER_SUPPLY_HEALTH_COLD:
			shbatt_api_kernel_battery_log_info_event(SHBATTLOG_EVENT_CHG_COLD_STOP_ST, 0);
			break;
		case POWER_SUPPLY_HEALTH_WARM:
			shbatt_api_kernel_battery_log_info_event(SHBATTLOG_EVENT_CHG_HOT_FAST_ST, 0);
			break;
		case POWER_SUPPLY_HEALTH_COOL:
			shbatt_api_kernel_battery_log_info_event(SHBATTLOG_EVENT_CHG_COLD_FAST_ST, 0);
			break;
		case POWER_SUPPLY_HEALTH_GOOD:
			shbatt_api_kernel_battery_log_info_event(SHBATTLOG_EVENT_CHG_FAST_ST, 0);
			break;
		default:
			break;
		}
...
#ifdef CONFIG_BATTERY_SH
#define FCC_CFG_MASK 0x1F
static int get_prop_charging_current_max(struct smbchg_chip *chip)
{
	int rc;
	u8 reg;

	rc = smbchg_read(chip, &reg, chip->chgr_base + FCC_CFG, 1);
...
#ifdef CONFIG_BATTERY_SH
	if ((!chip->thermal_mitigation) || (!chip->thermal_mitigation_hvdcp_vol_down) ||
		(!chip->thermal_mitigation_hvdcp_vol_up)){
#else
...
#ifdef CONFIG_BATTERY_SH
	if (prev_therm_lvl > chip->therm_lvl_sel){
		/* hvdcp lower direction */
		chip->thermal_limit_voltage = chip->thermal_mitigation_hvdcp_vol_down[chip->therm_lvl_sel] * 1000;
	}else{
		/* hvdcp upper direction */
		chip->thermal_limit_voltage = chip->thermal_mitigation_hvdcp_vol_up[chip->therm_lvl_sel] * 1000;
	}
...
#ifdef CONFIG_BATTERY_SH
	union power_supply_propval property = {0,};
/* COORDINATOR KK_AC120 BUILDERR MODIFY start */
//	int rc = 0;
/* COORDINATOR KK_AC120 BUILDERR MODIFY end */
#endif	/* CONFIG_BATTERY_SH */
	...
	case POWER_SUPPLY_PROP_CHARGING_ENABLED:
		...
		if (rc < 0) {
			...
			if(shbatt_api_is_disable_usb_charging() && property.intval == POWER_SUPPLY_TYPE_USB){
				smbchg_usb_en(chip, val->intval, REASON_POWER_SUPPLY | REASON_USER);
...
static irqreturn_t usbin_uv_handler(int irq, void *_chip)
{
...
#ifdef CONFIG_BATTERY_SH
				smbchg_usb_en(chip, 1, REASON_POWER_SUPPLY | REASON_USER);
	...
	pr_smb(PR_STATUS, "[S] chip->usb_present = %d usb_present = %d\n",
			chip->usb_present, usb_present);

	rc = smbchg_read(chip, &reg, chip->usb_chgpth_base + RT_STS, 1);
	if (rc < 0) {
		dev_err(chip->dev, "[P] Couldn't read usb rt status rc = %d\n", rc);
		goto out;
	}

	pr_smb(PR_STATUS, "[P] reg:0x%02x.\n", reg );

	if( reg & USBIN_UV_BIT )
	{
		if( chip->usb_psy == NULL )
		{
			dev_err(chip->dev, "[P] chip->usb_psy is NULL.\n");
			goto out;
		}
		...
		if( rc < 0 )
		{
			dev_err(chip->dev, "[P] Couldn't read POWER_SUPPLY_PROP_TYPE. rc:%d\n", rc);
			goto out;
		}
		/* UVLO or cable removed */
		pr_smb(PR_STATUS, "[P] UVLO occurs: prop.intval:%d.\n", prop.intval );
		switch( prop.intval )
		{
		case POWER_SUPPLY_TYPE_USB_HVDCP:
		case POWER_SUPPLY_TYPE_USB_DCP:
			if( chip->usb_present && !usb_present ) {
				...
				smbchg_usb_en(chip, 1, REASON_POWER_SUPPLY | REASON_USER);
				power_supply_charging_enabled_flg = false;
				pr_smb( PR_STATUS, "[P] UVLO occurs: DCP or HVDCP removed.\n" );
			}
			else
			{
				pr_smb( PR_STATUS, "[P] UVLO occurs: other state.\n" );
			}
			break;

		default:
			pr_smb( PR_STATUS, "[P] UVLO occurs: other charger=%d.\n", prop.intval );
			break;
		}
	}
	else if( reg & USBIN_SRC_DET_BIT )
	{
		if( !chip->usb_present && usb_present )
		{
			/* UVLO recovery */
			chip->usb_present = usb_present;
			handle_usb_insertion(chip);
			pr_smb( PR_STATUS, "[P] UVLO recovery: UVLO recovery.\n" );
		}
		else
		{
			pr_smb( PR_STATUS, "[P] UVLO recovery: other state.\n" );
		}
...
static irqreturn_t src_detect_handler(int irq, void *_chip)
{
	...
	if (reg) {
		if (!chip->usb_present && usb_present) {
			...
			handle_usb_insertion(chip);
#ifdef CONFIG_BATTERY_SH
			pr_smb( PR_STATUS, "[P] SRC_DET ON: USB inserted.\n" );
		}
		else
		{
			pr_smb( PR_STATUS, "[P] SRC_DET ON: USB other state.\n" );
#endif	/* CONFIG_BATTERY_SH */
		}
							...
							&prop)) {
#ifdef CONFIG_BATTERY_SH
		pr_smb(PR_STATUS, "[P] SRC_DET OFF: prop.intval:%d.\n", prop.intval );
#endif	/* CONFIG_BATTERY_SH */
		if (((prop.intval == POWER_SUPPLY_TYPE_USB_CDP) ||
			...
			chip->aicl_irq_count = 0;
#ifdef CONFIG_BATTERY_SH
			pr_smb(PR_STATUS, "[P] CDP or SDP removed.\n" );
		}
		else
		{
			pr_smb( PR_STATUS, "[P] other charger: other state.\n" );
#endif	/* CONFIG_BATTERY_SH */
		}
	}
...
static int smb_parse_dt(struct smbchg_chip *chip)
{
...
#ifdef CONFIG_BATTERY_SH
	if (of_find_property(node, "qcom,thermal-mitigation-hvdcp-vol-up",
					&chip->thermal_levels)) {
		chip->thermal_mitigation_hvdcp_vol_up = devm_kzalloc(chip->dev,
			chip->thermal_levels,
			GFP_KERNEL);

		if (chip->thermal_mitigation_hvdcp_vol_up == NULL) {
			dev_err(chip->dev, "thermal mitigation kzalloc() failed.\n");
			return -ENOMEM;
		}
		...
		chip->thermal_levels /= sizeof(int);
		rc = of_property_read_u32_array(node,
				"qcom,thermal-mitigation-hvdcp-vol-up",
				chip->thermal_mitigation_hvdcp_vol_up, chip->thermal_levels);
		if (rc) {
			dev_err(chip->dev,
				"Couldn't read threm limits rc = %d\n", rc);
			return rc;
	...
	if (of_find_property(node, "qcom,thermal-mitigation-hvdcp-vol-down",
					&chip->thermal_levels)) {
		chip->thermal_mitigation_hvdcp_vol_down = devm_kzalloc(chip->dev,
			chip->thermal_levels,
			GFP_KERNEL);

		if (chip->thermal_mitigation_hvdcp_vol_down == NULL) {
			dev_err(chip->dev, "thermal mitigation kzalloc() failed.\n");
			return -ENOMEM;
		}
		...
		chip->thermal_levels /= sizeof(int);
		rc = of_property_read_u32_array(node,
				"qcom,thermal-mitigation-hvdcp-vol-down",
				chip->thermal_mitigation_hvdcp_vol_down, chip->thermal_levels);
		if (rc) {
			dev_err(chip->dev,
				"Couldn't read threm limits rc = %d\n", rc);
			return rc;
