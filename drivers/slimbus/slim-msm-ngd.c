...
#ifdef CONFIG_SND_SOC_ES_SLIM
#include <linux/regulator/consumer.h>
#endif /* CONFIG_SND_SOC_ES_SLIM */
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-061 */
#ifdef CONFIG_SND_SOC_ES_SLIM
static int audience_power_supply(struct msm_slim_ctrl *dev)
{
	int ret = 0;
	struct regulator *vdd_core;

	vdd_core = regulator_get(dev->dev, "pm8994_l25");
	if (IS_ERR_OR_NULL(vdd_core)) {
		ret = PTR_ERR(vdd_core);
		pr_err("%s():%d Regulator pm8994_l25 get failed vdd_core ret=%d\n",
			__func__, __LINE__, ret);
		goto regulator_get_failed;
	}

	ret = regulator_set_voltage(vdd_core, 0, 1100000);
	if (ret) {
		pr_err("%s():%d Regulator pm8994_l25 set voltage failed ret=%d\n",
			__func__, __LINE__, ret);
		goto regulator_set_voltage_failed;
	}

	ret = regulator_enable(vdd_core);
	if (ret) {
		pr_err("%s():%d Regulator pm8994_l25 enable failed ret=%d\n",
			__func__, __LINE__, ret);
		goto regulator_enable_faild;
	}

	ret = regulator_set_mode(vdd_core, REGULATOR_MODE_NORMAL);
	if (ret) {
		pr_err("%s():%d Regulator pm8994_l25 set mode failed ret = %d\n",
			__func__, __LINE__, ret);
		goto regulator_set_mode_failed;
	}

	regulator_put(vdd_core);
	return 0;
regulator_set_mode_failed:
	regulator_disable(vdd_core);
regulator_enable_faild:
...
#ifdef CONFIG_SH_AUDIO_DRIVER /* 14-061 */
#ifdef CONFIG_SND_SOC_ES_SLIM
	ret = audience_power_supply(dev);
	if (ret) {
		pr_err("%s():%d Audience Power Supply failed ret=%d\n",
			__func__, __LINE__, ret);
	} else {
		pr_err("%s():%d Audience Power Supply successful\n",
			__func__, __LINE__);
	}
#endif /* CONFIG_SND_SOC_ES_SLIM */
#endif /* CONFIG_SH_AUDIO_DRIVER */ /* 14-061 */
